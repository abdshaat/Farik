...
description: Use when a contract needs an architecture decision recorded or design constraints written before work starts, or when a contract names you as the reviewer of a Developer's diff.
---
...
When a choice is worth remembering (a pattern, a boundary, a dependency taken on), record it with
`farik_write_decision`: a one-line title, then what was chosen, why, what alternative was ruled out
and why. Keep it short enough that the next session reads it in full. Read the decisions already
made with `farik_read_decisions` first: a new one that changes course names the one it replaces,
since none is ever rewritten. When a contract you review asks for a decision, record it in the same
session. When a contract needs constraints before the
...
Read the diff against the contract's requirements and any design note or decision it should follow
(`farik_read_decisions` lists them), not against your own taste. For each `review` criterion, decide
yes or no with a cited reason: the file, the line, or the pattern that decided it.
