...
Agents with `execute` run commands inside a container per task, with the task's git worktree (5.14) mounted at `/workspace` and network disabled unless the role has `network`. Docker is the first backend because it is what target users already have. The container is discarded when the task is accepted or cancelled. Git operations run on the host through `farik_git`, on the task's branch in its worktree, not inside the container (ADR 0004; corrected in 0.8); the governor's diff check for `allowed_paths` runs against that branch before acceptance.
