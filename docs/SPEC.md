...
`farik contract new --brief <text>` or `--from <issue>` (added in 0.8). The request's title is the brief's first line, cut at 120 characters, or the issue's title; its intent is the whole brief, or the issue's title, body, and address, and a brief under twenty characters, the schema's least intent, is refused before anything is filed. Every other field is a placeholder the Product Manager replaces while refining it: one item in scope and one out, a requirement, a `review` criterion, both roles Software Developer, risk `low`, the team's cap on a task's budget, and an allowed path no file matches. The request is filed as the human's, sized when `--size` is given, and then worked, in this process, with the triage and refining rules alone and for that request alone (8.2): each event about it is printed as `farik task show` prints it, and each question the Product Manager asks is printed with an `answer>` prompt, whose reply is recorded as the answer. The end of the input or Ctrl-C at the prompt leaves the question open for `farik answer`. Once nothing is left to do it prints the contract, its readiness (passed; failed with each failure; passed the structural checks, when it waits for the user's approval; or not judged yet, and why), what waits on the user, and, with `--lock`, takes the contract, last, so that the Product Manager writes first. While another process drives the project, the request is filed and sized and left to that process, and `--lock` is refused before anything is filed.
