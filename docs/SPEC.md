...
The daemon's `PreToolUse` answer (added in 0.8). A call is denied, with a reason that starts with its kind, when its session is not one the daemon registered (`unknown_session`), when the agent is not active (`agent_not_active`), when the session has made its `max_tool_calls` (`tool_call_limit`, counted by the daemon, one per allowed call), when the tool is neither one of Farik's nor a built-in with a tier (`tool_not_allowed`; `Read`, `Glob`, `Grep`, `LS`, and `ToolSearch` need `read`, `Edit`, `Write`, `MultiEdit`, and `NotebookEdit` `write_workspace`, `WebFetch` and `WebSearch` `network`, and no other built-in has one), when a built-in's path is outside the session's worktree (`path_outside_workspace`), and otherwise by the check of 5.6. A path is resolved against the worktree and through its symlinks, so a committed link that points out of the worktree is judged by where it points; a read outside it is denied as a write is, because a session's files are its worktree's and nothing else on the machine (8.6). A `Glob` whose pattern is absolute or climbs with `..` is denied the same way. So is a path or a `Glob` pattern with a part that starts with `~` (`~`, `~/x`, `~user/x`, `src/~x`), because Claude Code expands a leading `~` to a home directory before the tool runs, and the hook would otherwise judge a path under the worktree that the tool never touches. A decision the log cannot record is a deny (`record_failed`), because a tool call the log cannot vouch for is one the governor cannot either. A decision whose team file or contract cannot be read is a deny (`team_unreadable`, `contract_unreadable`), for the same reason. The hooks fail closed: `farik hook pre-tool-use --daemon <path>` prints a deny, with a reason that starts `hook_failed`, whenever it cannot get an answer (no daemon file, no daemon, a daemon that does not answer within ten seconds, an answer that is not a decision), and exits 2 with the reason on standard error if it fails itself, since exit 2 is the other answer Claude Code treats as blocking; the ten seconds are well under Claude Code's own hook timeout, after which it would let the call through. `farik hook post-tool-use` prints nothing and never blocks, because the call it reports is over.

The session's process (added in 0.8). It is run in the task's worktree with `--session-id <id>`, so that every hook names the session Farik registered; its system prompt is written to `.farik/local/sessions/<id>/system-prompt.md` and kept after the session, as the exact text the session was told; and its MCP config is a file beside it, `mcp.json`, readable by its owner alone, because it holds the daemon's token and a process's arguments are readable by every user of the machine. The user's own settings are not loaded (`--setting-sources ""`), so neither the user's nor the repository's hooks, permissions, or `CLAUDE.md` reach a Farik session; its settings are Farik's alone, passed with `--settings` and built at each session start from the team file as it is then: the two hooks, and a `Read(<glob>)` deny rule for each protected path, which Claude Code also holds `Grep` and `Glob` to. The session pays with either credential the user has, an API key (`ANTHROPIC_API_KEY`, preferred when both are set) or a subscription token (`CLAUDE_CODE_OAUTH_TOKEN`), and its environment is that one variable and `PATH`, `HOME`, `USER`, `LANG`, `TERM`, and `TMPDIR`, nothing else inherited, so a base URL, a provider switch, or another secret in the user's shell cannot change which account pays or reach the session. `--max-turns` is the session's tool-call limit plus one, a backstop to the daemon's count. The session takes one message; a later one is a `resume` of it. It ends at its `result` line, and is killed with all its process group when its caller aborts it or at its wall-clock limit, ending `aborted` or `limit`, whether or not its caller is still reading its events; a process that exits without a result ends `error` with the tail of its standard error. Whatever way it ends, its process group is killed once it is over, so nothing it started outlives it, and so it is if the daemon's runtime shuts down under it. The log records `session.started` with the purpose, model, and effort, and `session.ended` with the reason and what the program said. The oldest Claude Code Farik runs on is 2.1.272.
