...
The `git_local` tier is enforced at the tool boundary and by the first-word check in `farik_exec`, not by the container. An agent with `execute` but without `git_local` (the Architect on a spike) can still commit inside the container by wrapping git in a shell script. That commit lands on the task branch, where the reviewer's diff and the path check see it, and it is the accepted residual of this decision. The `git_remote` tier has no such residual because the container holds no credentials; that holds only with Docker.

Amended 2026-09-22, from the phase 3 step 02 landing review: in no-sandbox mode a command runs on the host as the user, with `HOME` passed through, so it can read the user's credential files (`~/.git-credentials`, `~/.ssh`, `~/.claude/.credentials.json`, the `gh` configuration) and push through a git wrapped in a shell script, which the first-word check does not see. Clearing the environment keeps credentials held in variables out, not those held in files. In no-sandbox mode the `git_remote` tier is enforced at the tool boundary only, and the credentials on disk are exposed to every command; that is part of what no-sandbox mode gives up, and its warning says so (`docs/SPEC.md` 8.3).

Amended 2026-09-22 by ADR 0012: `git_remote` governs what an agent's session may do, and nothing else. When Farik pushes accepted work or opens a pull request under the team's `integration` policy (`docs/SPEC.md` 5.14), no agent is acting, so that push is not checked against any agent's tiers. It runs on the host with the user's own git and `gh` credentials, which never enter a container, as above.
