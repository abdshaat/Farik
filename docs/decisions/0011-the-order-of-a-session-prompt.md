...
Skills are written into the `Role` section rather than passed as a skills folder. Inside the `Role` section every Markdown heading outside a fenced code block moves down two levels in the role's `system.md` and three in a skill's body, to `######` at most, so no role file can write a heading at the level of Farik's sections (amended 2026-09-22 by the step 10 landing review, which found the shipped roles' `## Your tools` printed beside Farik's own `## Your tools`).
...
The caps are guesses. A notebook past 32 KiB loses its end, which is its newest part if the agent appends, until phase 4 step 05 gives memory its own cap and refresh; a contract past 32 KiB loses its tail from the model's view, and with it fields the governor still judges. The role's own headings are rewritten one way, demoted, so a role author writes `#` and `##` as in any Markdown file and the prompt still has Farik's eleven `## ` headings and no others; a heading at `#####` or deeper flattens to `######`, and a heading indented as code or written in a fence is left alone.

`Your memory` now always opens with a line Farik writes, outside the `untrusted` block, saying how full the notebook is against `policy.memory_cap_tokens` (`<n> of <cap> tokens.`, with a prune sentence past 80 percent for a session offered `farik_write_memory`), so the section is written even for an empty notebook; and the notebook is cut at `cap × 4` characters in place of the fixed 32 KiB above, so a notebook within its cap is never cut (amended 2026-09-24 by phase 4 step 07).
