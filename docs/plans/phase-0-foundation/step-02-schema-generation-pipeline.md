...
Cargo.lock                                              modifies (generated by cargo): pins the new dependencies
...
- [x] Generate:
  ...
  The module exports `Role`, `ExitCriterion`, `ExitCriterionVerification` (five positional variants), `FarikTaskContract`, and a newtype for every `pattern` and length constraint (`ExitCriterionId`, `ExitCriterionSatisfiesItem`, `ExitCriterionText`, `FarikTaskContractDependenciesItem`, `FarikTaskContractId`, `FarikTaskContractIntent`, `FarikTaskContractParent`, `FarikTaskContractRequirementsItemId`, `FarikTaskContractRequirementsItemText`, `FarikTaskContractTitle`). If the hashes differ, the schema on `main` or a pinned crate version has changed since this plan was written; stop and update the plan.
