...
TOOLCHAINS        a marker file -> a name and where its commands come from
...
crates/store/src/git.rs                       modifies: declares fixtures, adds root and tracked_paths, renames the splitter
...
- [ ] Write the failing tests. In `crates/store/tests/git.rs`, insert before `fn keeps_a_path_whose_name_begins_with_a_space`:
