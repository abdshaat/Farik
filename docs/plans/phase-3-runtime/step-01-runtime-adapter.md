...
- Tool output is a `String`: a `content` string as it is, a `content` array as its `text` blocks joined with newlines. A `tool_result` with no `content` is empty output, because a tool may answer nothing; a `tool_use` with no `input` is `RuntimeError::Protocol` (added 2026-09-22 by the landing review, which found the null the code had put there instead).
