...
- Refusals: `docker version` failing, or the program missing, is `SandboxError::DockerUnavailable`; `docker image inspect <image>` failing is `ImageMissing { image }`, and nothing is pulled or built here; any other `docker run` failure is `ContainerFailed { detail }` with docker's stderr. A worktree path that is relative or holds a `,` is `ContainerFailed` naming it before docker is asked anything, because `--mount` splits its value on commas and a relative `src` is not a bind mount (landing review). A `docker exec` whose stderr has a line starting `Error response from daemon` that says `No such container` or `is not running` is `ExecError::ContainerGone` (only docker's own line, so a command that prints those words is not mistaken for a lost container); the process not starting at all is `ExecError::SpawnFailed`.
- `discard` is `docker rm -f` for the container and nothing for the host. A container already gone (docker's `No such container`) counts as discarded, since that is all `discard` asks, and recovery may discard one a restart took (landing review).
- `SANDBOX_IMAGE` is `farik/sandbox:<crate version>` from `env!("CARGO_PKG_VERSION")`. The `Dockerfile` at `crates/runtime/sandbox/Dockerfile` installs Node 24, pnpm (pinned, 10.34.5, the newest 10.x on 2026-09-22; landing review), npm, python3, git, coreutils, and the Rust toolchain pinned in `rust-toolchain.toml`; it ships and is not built by CI (revision 8). The container runs as the user's uid, which has no passwd entry, so the image sets `HOME=/tmp`, puts `CARGO_HOME` and `RUSTUP_HOME` under `/usr/local` and makes both world-writable, so that `cargo`, `npm`, and `pnpm` can write their caches.
