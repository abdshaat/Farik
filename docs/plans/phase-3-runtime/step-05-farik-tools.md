...
- Git tools work on the task's worktree, `.farik/local/worktrees/<id>`: status is the new `Git::status(worktree) -> String` (`git status --porcelain`), diff is `Git::diff` from the integration branch (resolved as step 04 resolves it) to `farik/<id>`, commit is the new `Git::commit(worktree, message, paths) -> sha` (`git add -- <paths>` then `git commit -m`; changed 2026-09-23 by the final review, which found that git read a named path as a pathspec, so `src/*` or a directory staged a protected file the path checks never saw: the add is `git --literal-pathspecs add -- <paths>`, and `farik_git_commit` refuses a path that is a directory in the worktree with `path_is_a_directory`, rather than expanding it to its files and checking each, because an agent that names its files commits what it meant and a refusal is one check where an expansion is a second walk of the tree to keep in step with git's), push is the new `Git::push(remote, branch)` of `farik/<id>` to `origin`. A session with no task is refused.
...
- `commits_a_named_path_as_a_path_and_never_as_a_pattern` (store; added 2026-09-23 by the final review) — `src/*`, `src/*.ts`, and `:(glob)src/**` each fail as matching nothing, `HEAD` does not move, and nothing is left staged.
- `refuses_to_commit_a_directory_that_holds_a_protected_file` (added 2026-09-23 by the final review) — with `src/login/**` allowed and `src/login/k.pem` in the worktree, `src/login` and `src/login/` are each refused `path_is_a_directory: <path> is a directory; name the files to commit`, nothing is committed, and `src/login/form.ts` named alone commits that file only.
