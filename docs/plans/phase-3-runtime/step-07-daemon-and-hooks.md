...
- Changed 2026-09-22 by the landing review: a `PostToolUse` of a session the daemon no longer answers for is still recorded as `tool.returned`, with the session's id and no agent or task, and answered 200 `{}`, because a session ended between its two hooks did run the tool its `tool.called` records, and the log should say what came back; `records_the_return_of_a_session_it_no_longer_answers_for` holds it.
- Changed 2026-09-22 by the landing review: the token is compared in a time that depends on the lengths alone (a hand-written XOR fold, no dependency), so that a refusal's timing says nothing about how much of a guess was right. `writes_the_daemon_file_and_removes_it_on_shutdown` no longer connects to the freed port after `shutdown`, which raced with any test that bound it; `shutdown` answering `Ok` means the server's task joined.
- Changed 2026-09-22 by the landing review: `docs/SPEC.md` 8.2 names the deny kinds `team_unreadable`, `contract_unreadable`, and `hook_failed`, which Task 1 and Task 4 introduced without writing them there.
- Changed 2026-09-23 by the landing review of step 14: `SessionRegistration` carries `farik_tools`, the Farik tools the session was given (`SessionSpec::farik_tools`), because nothing read that list and a triage session could call any Farik tool its agent's tiers allowed, against 5.16's one tool. `decide_pre_tool_use` denies a Farik tool outside it as `tool_not_in_session`, after `tool_not_allowed` and before `evaluate_tool_call`, for every purpose; the MCP server's `CallingSession` carries the list, `list_tools` lists only those tools and the permission tool, and `call_tool` answers any other with the same refusal as an error result. `docs/SPEC.md` 8.2 names the kind. Tests: `denies_a_farik_tool_the_session_was_not_given` (a triage session's `farik_write_contract` and a board-only session's `farik_write_note` denied, its `farik_triage_request` allowed) and `serves_a_session_only_the_tools_it_was_given` (a triage session lists `farik_triage_request` and `permission`, and a call of `farik_write_contract` is refused with no `contract.written`), each seen to fail before the check existed. The fixture's `register` gives every Farik tool, so that the tiers alone decide as before; `register_with_tools` gives a list.
- Changed 2026-09-23 by live testing: Claude Code 2.1.280 speaks MCP `2026-07-28` to the daemon (no `initialize` and no MCP session: `server/discover`, then every request with the version in its `_meta` and its headers), and that revision requires a list result's `ttlMs` and `cacheScope`, which `rmcp` 3.3.0 leaves out unless the handler sets them. The program rejected the list and the session had no Farik tools at all (its `init` line listed no `mcp__farik__` tool), so the model wrote its tool calls as text. `list_tools` now answers `ttlMs` 0 and `cacheScope` `private`: private because the list is the session's own, filtered by its `farik_tools`; 0, "always refetch", because a session ends and its list with it, and a refetch over loopback costs nothing worth caching for. Both fields go to every peer, whatever version it negotiated: `rmcp` clears only `resultType` for a peer before `2026-07-28`, and the older schemas let a result carry any other field, so one code path serves both. `call_tool`'s and `server/discover`'s answers were checked against the same revision and need nothing: `rmcp` adds `resultType` `complete` to a call's result for a `2026-07-28` peer, and its default discover answer already carries `ttlMs` 0 and `cacheScope` `private`; `initialize` is legacy-only, and negotiates a legacy version. `answers_a_modern_client_as_protocol_2026_07_28_requires` speaks as the program does, and the live test now calls `farik_read_board` (step 08).
