...
- [ ] `feat(runtime): serve the hooks on a local port behind a token`
...
- [ ] `feat(runtime): serve farik tools and the permission tool over mcp`
...
- [ ] `feat(cli): add the farik hook commands`
