...
- [x] `feat(store): write contract and library yaml through one public helper` (the writers and their test)
...
- `says_exactly_where_a_cap_that_is_not_whole_kib_cut` (added by the landing review) — caps of 10 and 1,536 bytes say `[cut at 10 bytes]` and `[cut at 1536 bytes]`; a cap of 2,048 says `[cut at 2 KiB]`.
- `refuses_a_source_that_could_break_out_of_its_attribute` (added by the landing review; debug builds) — an empty source, one holding `"`, `>`, `<`, a space, a hyphen, or a capital panics; `diff`, `review_note`, `project_scan` do not.
- `names_every_tier_as_the_wire_spells_it` (added by the landing review) — the tool list's tier name of each `PermissionTier` equals its serde name, now that the name is a `match` rather than a serialisation that could fall back to `""`.
