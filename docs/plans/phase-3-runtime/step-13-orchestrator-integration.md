...
- Changed 2026-09-23 by the landing review: a `SandboxFactory::remove` that fails during cleanup (rule 1, and recovery) no longer fails the tick, which stopped every tick while docker was down: the worktrees are left as they are, so the task stays eligible and a later tick tries again, and the rule does not act, so the tick goes on to the other rules. Nothing reports the failure (the orchestrator has no log to write it to); the task's worktree kept is what shows it. Tested with `UnremovableSandboxFactory` (`fixtures.rs`), whose first removals fail: a ready task still gets its plan session, and the next tick cleans up.
