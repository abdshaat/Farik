...
- Changed 2026-09-23 in execution (Task 1): the sentence for a refused contract write (`contract_write`) moved from `crates/cli/src/refusal.rs` to `farik-store::requests`, which `hold_contract` refuses with, and the command line re-exports it, so the phase 2 tests read the same words. `triage_by_human` takes the plan's eight arguments under `#[allow(clippy::too_many_arguments)]`. Both functions refuse a task the board does not hold, with the command line's old sentence.
- Changed 2026-09-23 in execution (Task 2): `question.answered`'s `question_id` is a `NonZeroU64` in the generated type, because the schema's minimum is 1. `command.schema.json` carries its own `taskStatus` list, since one schema never references another. The board keeps a count, `open_questions`, in the migration's column, and `TaskProjection` shows it as `waiting_on_human`; the struct carries `#[allow(clippy::struct_excessive_bools)]`, its flags being what a board filters by. A non-approval `escalation.raised` clears `awaiting_approval` as well as a move does, so the flag is the last escalation's since the last move, which is what the backfill reads.
- Changed 2026-09-23 in execution (Task 3): the human's reason is the escalation's words after the rejection's and the blocker's and before Farik's unrunnable criterion's. The results an acceptance stands for carry the evidence `human.accepted at seq <n>` for `review` criteria too.
- Changed 2026-09-23 in execution (Task 4): the store's refusals reach the human as `lock_refused: <sentence>` and `triage_refused: <sentence>`, kinds the plan did not name; every command about a task answers `NotFound` for a task the board does not hold before anything else. `EscalationResolve`'s move records the message as the move's `reason` as well as on `escalation.resolved`. `CommandReport::events` is the task's events after the command began (the log's own seqs), and `agent.updated` plus the moves for `AgentUpdate`. The governor has no human row from `in_progress` to `accepted`, so `moves_a_task_for_the_human` reads `transition_refused: no such transition: ...`.
- Changed 2026-09-23 in execution (Task 5): the hook's denial of an agent not in the team file at all asks the stop with "agent retired by the user", as a retirement's; the first stop reason a session is given is the one kept. The session loop takes the daemon's `Notify` before it reads the stop reason, so a stop between the two still wakes it; the tests spawn the command's task and wait on `UsageThenWaitAdapter::started_count`, a counter added to the fixture.
- Changed 2026-09-23 in execution (Task 6): a valid team always has an active Product Manager (`validate_team` refuses one without), so `acts_for_no_agent_that_is_paused` pauses `pm` beside an active second Product Manager `pm-2` and holds that `pm-2`, the first active one in team-file order, is who moves the draft; its second half is as planned. `judges_a_child_filed_whole_without_a_session` files FRK-2 by calling `farik_create_task` as `pm`, the call `plan_breaks_down_frk_1` makes, because the session that replays it is Task 7's rule; for the same reason rules 6 and 7 pass over an epic from this task, and Task 7 gives them the epic's own rules. An approval of a contract with words reaches the next session as `The human, approving the contract: <message>`, which the plan did not name. A refine session whose contract was judged not ready reports `Acted` with the governor's reasons, and the next tick starts the refine session.
- Changed 2026-09-23 in execution (Task 7): `waits_to_assign_a_second_epic_while_the_first_is_open` seeds a task under FRK-1, blocked, before its third tick, because rule 6 breaks down an `in_progress` epic with no live task before rule 8 is reached, so the plan's `Idle` holds only once FRK-1's breakdown has begun. `leaves_an_epic_alone_while_its_tasks_are_open` and `waits_for_an_epics_tasks_to_be_integrated` passed before this task's code, through Task 6's pass-over of an epic in rule 6 and rule 5's pass-over of a task with no active reviewer; they hold the behaviour now that the epic has its own rules. The epic's runs record each result before the next runs, in one blocking task that holds the worktree and the base sandbox; a criterion whose container went is recorded failed, and any other error the runs meet escalates the epic as step 12's unrunnable criterion does, while a base sandbox that cannot be made fails the tick. The breakdown's and close-out's first messages name `farik_create_task`, and the close-out lists the tasks inside `untrusted_block("tasks", ..)`, their titles being an agent's words. The epic's `review.recorded` carries the Product Manager's session on its envelope and no agent. `accepts_a_high_risk_task_after_the_human` lives in `rules.rs` beside the test it continues.
- Changed 2026-09-23 in execution (Task 8): `takes_one_request_to_an_accepted_epic` is in `crates/runtime/src/orchestrator.rs`'s tests rather than `crates/runtime/tests/one_epic.rs`, for step 12's reason: the harness and the tool runner are test-only inside the crate. It passed against Tasks 1 to 7's code, as an end-to-end test of finished parts does; it was seen to fail with the epic's criterion runs skipped (`criteria_not_run` refused the human's acceptance).

- Changed 2026-09-23 by the landing review: `SessionSpec::farik_tools` is enforced. The session's registration carries it, the hook denies any other Farik tool as `tool_not_in_session`, and the MCP server lists and calls only the session's own (the step 07 plan records the mechanism), for every purpose. `triages_a_request_on_the_cheaper_model` also holds that the daemon registered the triage session with `farik_triage_request` alone (`ExecutorWitness::given_tools`), seen to fail with the registration given every tool. Every transcript of steps 11 to 15 was replayed with the check in place and none calls a tool outside its session's list: the triage transcript calls `farik_triage_request` alone, and every other session is given the tools its agent's tiers allow, which the hook already held it to.
...
crates/runtime/tests/one_epic.rs                     creates: the end-to-end test, ignored (needs git); landed in orchestrator.rs's tests (Task 8's note)
...
- [x] `feat(runtime): triage requests and refine their contracts`
...
- [x] `feat(runtime): take an approved epic through its breakdown to acceptance`
...
- [x] `test(runtime): take one request to an accepted epic end to end`
