...
Status: draft (second readiness round pending; the first round's findings and the founder's epic review decision, ADR 0013, are folded in)
...
- An epic's result (ADR 0013, the founder, 2026-09-22). Rule 5 for an epic `reviewed_by_the_human`, in place of step 12's items 1 to 4:
  ...
  2. For each `command`, `test`, or `artifact` criterion with no governor result since the last move into `verifying`: step 12's per-criterion run, recorded as `criterion.recorded { run_by: reviewer, recorded_by: governor }`, `new_tests` none because an epic has no branch to compare. The runs happen on a detached worktree at `.farik/local/worktrees/<id>-base`, made with `Git::create_detached_worktree` at the integration branch's head after removing any left over, in a sandbox from `SandboxFactory::create_base`. The sandbox is discarded and the worktree removed after the last run, on every way out. Chose the `-base` name and sandbox because step 13's cleanup and `remove` already know them, and a `<id>` worktree would make `Transitions::work` look for a `farik/<id>` branch the epic does not have.
...
- `refuses_a_body_that_is_another_commands` — `question_answer` with a `task_integrate` body, and `human_accept` with `subject: approve`, are each refused with exactly one error, at `/body`, because the `oneOf` fails as a whole.
...
- `returns_a_failing_contract_with_its_failures` — the seeded failing write: the tick records `contract.evaluated { passed: false }` whose failure names `max_task_budget_usd`; the next tick's refine session has a first message containing that failure's text.
...
Files: `orchestrator/requests.rs`, `rules.rs`, `messages.rs`, `prompt.rs`, the plan transcripts, `docs/SPEC.md` (5.1, 5.4, 5.16)
