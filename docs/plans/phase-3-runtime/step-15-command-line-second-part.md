...
- Changed 2026-09-23 in execution (Task 6): the placeholder budget is the team's `max_task_budget_usd`, falling back to `DEFAULT_MAX_TASK_BUDGET_USD` where the rules carry none, which the shipped defaults never leave empty before step 17 replaces it with `placeholder_budget_usd`. The question is printed as `question <seq> from <agent>: <question>` before the first `answer> `; an answer's `said` is printed after it, and an answer `handle` refuses is printed as a refusal and the loop goes on. Under `--json` nothing but the final object goes to standard output (the tick lines are not printed), and a request handed to another process prints `{ task_id, status, handed_to }`. A question left open prints no contract or readiness line, and the final object's `readiness` is then `not_judged` with `why: "a question is open"`.
...
- [x] `feat(cli): write a contract with the product manager at the terminal`
