...
- Changed 2026-09-23 by the landing review: the store's metrics tests gain `counts_neither_a_rejection_nor_a_second_verification_as_first_pass` (a task rejected once and accepted by the human, and one verified twice and never rejected, beside one first pass) and an accepted epic in `says_none_for_every_rate_before_a_task_is_accepted`, so that the criteria share is `None` for want of an accepted task and not of criteria. `replays_an_older_project_into_the_new_counts` also writes a version 6 row for FRK-9, which the log never created, and holds that the migration's replay drops it. `accepted_project` in `crates/cli/tests/reading.rs` now gives every metric a value of its own (two accepted tasks, FRK-1 verified twice with the human unblocking it and one escalation, FRK-2 with a `command` and a `review` criterion and one escalation after acceptance, and costs of three purposes in three ISO weeks: 2 tasks, 50.0%, 1.50, $1.00 of which $0.50 at most per purpose, 66.7%, 3 weeks), and the JSON test compares the whole object; the costs are dated through a new shared helper, `record_on`, which `record_as` now calls with `at()`. The projection race `projects_every_event_when_several_processes_append_and_project_at_once` appends 50 events per process rather than 150, to lighten it on a loaded machine; the out-of-order hand-over it first found is held deterministically by `reads_what_it_was_handed_out_of_order_rather_than_stepping_over_it`.
