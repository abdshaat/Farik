...
- Changed 2026-09-23 in execution (Task 6): the sentence doctor and the start share is `doctor::unpriced(project) -> Result<Vec<String>, String>` (crate-private), which doctor prefixes with `.farik/team.yaml: ` and ends with ` (5.5)`, and the start prefixes with `warning: ` and ends with `.`. The start's credential line is printed by its caller once the start has succeeded (step 15), so the warnings come after the credential is chosen and before `serve`, and so before that line on standard error. Decided where the plan was silent: a shipped role file that breaks its schema (`RoleError::Invalid` from `unpriced_models`) is handled as an unreadable price table is, a finding in its own words for doctor and a refusal for the start, since every session of that role would fail on it. The tests' teams are `pm` and `dev` through the shared harness's `a_team_with`.
- Changed 2026-09-23 by the landing review: step 11's rule that a session is ended whatever its zero cost does has its test back. `farik_store::event_log::fixtures::refuse_appends_of(log, kind)` installs a trigger on the log's own connection (the orchestrator's harness keeps its log in memory, so no second connection could reach it) that refuses every append of one kind. `ends_a_session_that_cannot_start_when_its_zero_cost_is_refused` holds that the tick reports the failed start (`Err(Runtime)`, which failed first) and one `session.ended { reason: error }` is recorded with no cost; `ends_a_session_that_reported_no_usage_when_its_zero_cost_is_refused` holds `Err(Cost(Store))` and the same one end. With no daily budget and $25 spent today, `assigns_a_ready_task_whatever_the_day_cost_without_a_daily_budget` and `readies_a_written_contract_whatever_the_day_cost_without_a_daily_budget` hold that assignment and readiness still pass, and `farik-core` holds the same of an infinite remainder at both gates and of a $12 budget under `TeamRules::default()`. `unpriced_models` is tested with a Product Manager whose own model is the triage model, doctor's sentence with two agents on one unpriced model, and `session_model` with a role whose effort is `medium`.
...
- [x] `feat(cli): start a project with no dollar limit`
...
- [x] `feat(cli): warn of a model no price table prices`
