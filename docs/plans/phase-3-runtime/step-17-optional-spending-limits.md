...
Depends on: step 16 of this phase, a start gate: Task 1 does not begin until step 16's last commit is on this branch. Task 6 edits step 15's `crates/cli/src/start.rs`, Task 4 reads step 14's `TRIAGE_MODEL`, and Task 3 tests step 16's `Projections::metrics`. Also step 11 (`room`, the day's check before a session, commit 1340e5a); step 04 (`Transitions::context`); step 03 (`budget_state`, `record_session_cost`); phase 1 (`check_budgets`, `TeamRules`, the price table, on main); ADR 0015
...
- The Product Manager's skill says a task's budget is within the team's maximum "when the team sets one". Its line about the day's budget stays accurate as it is.
...
Files: `team.schema.json`, `budget.rs`, `team_rules.rs`, `transition.rs` (tests), `team.rs` (tests), `cost.rs` (`budget_state` and tests), `transitions.rs` (tests), `orchestrator/rules.rs` (tests), `SKILL.md`, `docs/SPEC.md`
...
- `starts_sessions_whatever_the_day_cost_without_a_daily_budget` (`orchestrator/rules.rs`): the setup of `starts_nothing_when_the_day_is_spent` with `budgets: {}`. The second tick starts FRK-2's implement session, and its report is not `Idle { why: "the team's daily budget is spent" }`.
...
Files: `event.schema.json`, `event.rs` (tests), `crates/store/tests/metrics.rs`
Produces: `CostRecordedBody::unpriced: bool`
Consumes: nothing from this plan

- `reads_a_cost_without_unpriced_as_priced` (`event.rs`): the `cost.recorded` fixture, which has no `unpriced`, parses with `unpriced == false`.
...
Files: `cost.rs`, `session.rs`, `orchestrator/session.rs`, `docs/SPEC.md` (5.5)
...
- `records_a_priced_model_as_priced` (`cost.rs`): the existing priced case's event has `unpriced: false`.
...
SPEC 3, in this commit: `farik doctor` also reports "an active agent's model that no price table prices".
