...
- Made: triage (spec 5.16) is the orchestrator's first move on a request. A user request (`farik contract new`, `farik task create`, the editor, `farik_propose_task`) creates a `draft` contract; the orchestrator starts a `triage` session on the cheaper model for the Scrum Master, or the Product Manager without one, whose only tool is `farik_triage_request { size: large | small, reason }`, which sets `kind` and emits `request.triaged`; the human overrules with `farik triage <id> large|small` (command `RequestTriage`) before refining starts, and a Product Manager that finds a small request is large calls `farik_triage_request` again with `large`, which turns the task into an epic and restarts refining. A small request is a standalone task (`kind: task`, no parent) that follows the ordinary lifecycle.
...
| 11 | Command line, second part | 5.13, 5.16, F6, F11, F14 | `farik run`, `farik plan`, `farik contract new` (creates a request for triage), `farik triage`, `farik approve`, `farik answer`, `farik integrate`, `farik stop`, `farik resolve`, `farik accept`, `farik cancel`, `farik task show` with events, diff, cost, and children |
...
| 04 | Board | F3, 5.14, 5.16 | Kanban of the lifecycle with tasks grouped under their epics, filters by agent, sprint, risk, and epic; waiting-on-triage, waiting-on-human, awaiting-approval, and awaiting-integration markers; overrule a triage; task detail with contract history, events, diff, notes, cost by purpose, and children; create an epic by hand; integrate |
