...
Status: draft, revision 4. The backend is Rust (ADR 0005); every interface in this plan is a Rust signature except the front end's. Phase 0 is fully decided and its step plans are written under `docs/plans/phase-0-foundation/`. Every product decision the plan raised (D1 to D20) was closed by the founder on 2026-09-15 and folded into the phase it governs; the closed decisions table at the end records each answer and where it lives.
Owner: project founder.
Last decision change: 2026-09-15, revision 4: the backend moves to Rust and the runtime drives the Claude Code program as a child process (ADR 0005); D7 is recorded as the founder clarified it (a Developer's work waits for a reviewer agent; the Product Manager asks the human to add one); D12 is closed without an assumption (the daemon is the Rust runtime itself, in-process in the desktop app).
...
- Execution model: agent sessions run on the host as Claude Code child processes; the program's shell tool is disallowed, its web tools are allowed only under `network`; command execution is a Farik tool backed by an `Executor` with a Docker implementation and a host implementation; git operations that need credentials or a tier are Farik tools on the host; governor decisions reach the program through `farik hook` commands that call the daemon: ADR 0004 (mechanism amended by ADR 0005). No-sandbox mode ships (D1): the host implementation is chosen per machine in `.farik/local/settings.yaml`, and every `farik run` and the setup screen warn when it is on.
...
| 02 | Definition of Ready and team rules | 5.3, 5.12 | `TeamRules` and its defaults; structural checks, rule checks, the reviewer-available check, and the judgment rubric as one function over a contract and a context, one test per rule |
...
| 08 | Gate predicates | 5.2, 5.11, 5.14 | Assignment (with dependency integration and the no-Scrum-Master rule), criteria recorded (with work committed), blocker written, blocker resolved, rejection reasons, contract write (locked and frozen), each returning `GateResult` |
...
| 06 | Command line | F2, F3, F11, F15, F16, 5.11 | `crates/cli`: `farik init`, `farik task create`, `farik task show`, `farik board`, `farik log`, `farik doctor`, `farik contract lock`, `farik contract unlock`, `farik rules show`, `farik criteria list` |

Interfaces this phase adds:

- Step 01 (`farik-protocol`): `enum EventKind` (this phase: `TaskCreated`, `ContractWritten`, `ContractLocked`, `ContractUnlocked`, `DriftDetected`, `ProjectScanned`, `TeamUpdated`, `CriteriaUpdated`; `snake_case` dotted names on the wire: `task.created`, `contract.written`, and the rest); `struct EventEnvelope { seq: u64, recorded_at: DateTime<Utc>, team_id: String, project_id: String, task_id: Option<TaskId>, agent_id: Option<String>, session_id: Option<String> }`; `struct FarikEvent { envelope: EventEnvelope, body: EventBody }` where `enum EventBody` has one variant per kind with its payload struct; `struct NewEvent` (an event without `seq`); `fn new_event(kind..., recorded_at, ids...) -> NewEvent`; `fn event_from_value(value: &Value) -> Result<FarikEvent, Vec<ValidationError>>`; `fn event_to_value(event: &FarikEvent) -> Value`; `enum Command` (this phase: `TaskCreate { contract: TaskContract }`); `trait Clock { fn now(&self) -> DateTime<Utc>; }`; `trait IdSource { fn session_id(&self) -> String; }`; `struct FixedClock`, `struct SequentialIds` for tests.
...
- Step 03: `struct TaskProjection { task_id, status, title, assignee_id, reviewer_id, risk, sprint_id, iteration, cost_usd, waiting_on_human: bool, awaiting_integration: bool, updated_seq }`; `enum CostScope { Task, Agent, Session, Sprint, Day }`; `struct CostProjection { scope, key, usd, input_tokens, output_tokens }`; `fn open_projections(log: &EventLog) -> Result<Projections, StoreError>`; `impl Projections { fn rebuild(&self) -> Result<(), StoreError>; fn apply(&self, event: &FarikEvent) -> Result<(), StoreError>; fn board(&self) -> Result<Vec<TaskProjection>, StoreError>; fn task(&self, id: &TaskId) -> Result<Option<TaskProjection>, StoreError>; fn costs(&self, scope: CostScope) -> Result<Vec<CostProjection>, StoreError>; fn cursor(&self) -> Result<u64, StoreError> }`.
...
Ends with: Milestone 0. On Farik's own repository, a Product Manager agent writes contracts for three real issues (one of them co-written and locked by the human), a Developer agent implements them in their own worktrees, a second Developer verifies, the Product Manager accepts, the harness metrics are printed, and a human reviewing the diffs and the event log agrees each task was done as contracted. Command line only.
...
- Made (D7): the reviewer of a task is the agent whose role is the contract's `reviewer_role`. For a Developer's work the preference is the Architect, then another Developer. When the team has neither, no Developer contract passes the Definition of Ready (`ReviewerAvailable`), and the Product Manager, told so by the readiness result, asks the human through `farik_ask_human` to add a reviewer agent. Milestone 0 therefore runs with two Developers. `REVIEWER_ROLE_FOR` and `default_reviewer_role` live in step 07 and the Product Manager's prompt uses them.
...
- Made (D8): `human_accepts_contracts` is a team policy with values `high_risk` (spec 5.2) and `all`; the shipped default is `high_risk`.
...
- Made: session purpose (`refine`, `plan`, `implement`, `verify`, `ceremony`, `conversation`) is stamped on `session.started` and on every `cost.recorded` event.
...
| 03 | Farik tools | 5.1, 5.2, 5.6, 5.7, 5.9, 5.11, 5.12, 5.13 | The Farik MCP tool set with tiers and handlers: read task, read board, read rules, read criteria, write contract (through the contract-write gate), request transition, assign task, record criterion result, write note, create task, declare blocked, ask human, exec (through `evaluate_command`), git; `Git` widened with `commit` and `push` |
...
| 10 | Orchestrator | 5.2, 5.4, 5.5, 5.7, 5.11, 5.14, 5.15, F1, F6 | The loop that moves tasks through the lifecycle with worktrees, sessions, governor decisions, reviewer resolution, integration, recovery, questions, agent pausing, escalations, and human commands; tested end to end with the recorded adapter |
| 11 | Command line, second part | 5.13, F6, F11, F14 | `farik run`, `farik plan`, `farik contract new`, `farik answer`, `farik integrate`, `farik stop`, `farik resolve`, `farik accept`, `farik cancel`, `farik task show` with events, diff, and cost |
...
- Step 07 (`farik-roles`): `struct RoleDefinition { id: Role, mandate, produces: Vec<String>, forbidden: Vec<String>, default_tiers: Vec<PermissionTier>, model: String, effort: Effort, session_limits: SessionLimits, system_prompt: String, skills: Vec<Skill> }`; `enum RoleError { NotFound { role_id }, Invalid { role_id, detail } }`; `fn load_role(role: Role) -> Result<RoleDefinition, RoleError>` (definitions embedded with `include_str!`; a user override under `.farik/roles/<role_id>/` is read by the store and merged by the runtime); `REVIEWER_ROLE_FOR: &[(Role, &[Role])]` (Developer's work to Architect, then another Developer; Architect's to Product Manager; Product Manager's contracts to Scrum Master; Marketing's to Product Manager); `fn default_reviewer_role(team: &Team, assignee: &Agent) -> Option<Role>` (the first preferred role with an active agent other than the assignee).
...
Ends with: from the command line, a team of five runs a full sprint on Farik's repository: planning in the channel, tasks contracted, assigned by the Scrum Master, implemented, reviewed by the Architect, accepted by the Product Manager, a standup summary at each boundary, a review and a retro at the end, with memory and decisions written to `.farik/`.
...
- Made: channel sessions carry `farik_create_task` and `farik_post_message`, nothing else that changes a task (spec 5.9).
...
- Step 01: the three role directories; the readiness context gains `requires_judgment_review` true when an active Scrum Master exists (D2); a `farik_record_judgment` tool for the Scrum Master; Scrum Master `plan` sessions take over assignment from the Product Manager when one is active.
- Step 02: `struct Sprint { id, started_at, ended_at: Option, budget_usd, task_ids, status: Open | Closed }`; `ProjectFiles` gains `read_sprint`, `write_sprint`, `list_sprints`; `Projections` gains `fn sprint(&self, id)`; commands added: `SprintStart`, `SprintEnd`; event kinds added: `sprint.started`, `sprint.ended`; `check_assignment` receives WIP counts from the sprint's board.
- Step 03: `struct Message { author_id: AuthorId, text, mentions: Vec<String>, refs: MessageRefs, kind: Reaction | Ambient | Ceremony | User }` as the payload of `message.posted`; `fn posting_triggers(event: &FarikEvent, team: &Team) -> Vec<PostingTrigger>`; `fn ambient_allowance(projections, sprint_id, agent_id) -> Result<u32, StoreError>`; `RuntimeAdapter` gains `fn complete(&self, model: &str, prompt: &str, max_tokens: u32) -> Result<Completion, RuntimeError>` for cheap one-shot calls; `fn summarize_channel(messages: &[Message], previous: Option<&str>, adapter: &dyn RuntimeAdapter) -> Result<String, RuntimeError>`; command added: `MessagePost`; tools added to channel sessions: `farik_post_message`, `farik_create_task`.
...
| Step | Name | Spec | Delivers |
|---|---|---|---|
| 01 | MCP per agent | F9, 3, 5.6, 8.6 | Server configuration (stdio and remote), tool listing, tier tagging, keychain credentials, loading into sessions; per-call human approval of `external_effect` tools from the board and from `farik approve` |
...
- Already made in spec 0.3 by this plan's pull request: section 8 for the Rust backend and the Claude Code child process, 5.1 and 5.3 for the reviewer rule (D7), and the founder's other decisions (D3, D4, D5, D6, D9, D10, D11, D15, D18) in sections 1, 3, 5.2, 5.5, 5.9, 8.4, 11, 12, and F1.
...
| D7 | A Developer's reviewer is the Architect, else another Developer; with neither, the contract fails readiness and the Product Manager asks the human to add a reviewer agent. | phase 1 step 02 (`ReviewerAvailable`); phase 3 step 07; spec 5.1, 5.3, and 11 |
