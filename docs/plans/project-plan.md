...
This document divides the project into phases and each phase into steps. A phase ends in something a person can use or verify. A step is one pull request with its own plan at `docs/plans/phase-<n>-<name>/step-<nn>-<name>.md`. Steps run in the order listed; a step depends only on steps above it. The rules are in `docs/standards/workflow.md` phase 2 and ADR 0003.
