...
| Schema validation | JSON Schema 2020-12 at the wire boundary; Rust types generated by `typify` through `cargo xtask generate`, validated by the `jsonschema` crate before deserialising | TypeScript types generated by `json-schema-to-typescript` | The schema in `docs/schemas/` is the source of truth. Generated files are committed and checked for staleness by the check command. |
...
| Generated Rust | `crates/<name>/src/generated/<schema>.rs` next to a copy of its schema; written only by `cargo xtask generate` | `crates/core/src/generated/task_contract.rs` |
